...
- 정보량의 최소 기본 단위로 이진수 체계(0, 1)의 한 자리를 나타내는 단위는? : `비트`
- 3바이트는 총 몇 비트로 구성되어 있는가? : `24bit`
...
- `프로그램` : 컴퓨터에 주어지는 일련의 명령문 집합체
- `프로세스` : 컴퓨터 내에서 실행 중인 프로그램
- `스레드` : 다중 작업 환경에서 병행 처리할 때 효율적인 처리를 위해 분할한 처리 단위
- `멀티 프로세스` : 여러 흐름 동시 진행, 각 프로세스는 독립적, 각각 **별개의 메모리** 차지
- `멀티 스레드` : 여러 흐름 동시 진행, 프로세스 내 **메모리 공유** (전환 속도 빠름)
- `임계 구역` : 둘 이상의 스레드가 동시 접근하면 안되는 공유 자원에 접근하는 코드
- `임계구역 해결을 위한 3가지 요구 조건`
  - `상호배제` : 한 프로세스만 임계구역에 진입
  - `진행` : 임계구역에서 작업 중인 프로세스 없다면, 프로세스 중 하나 적절히 선택해 임계구역에 진입
  - `제한된 대기` : 한 프로세스가 임계구역 진입 요청 후 기다리는 데 한계가 있음
- 모니터 (모니터링 락)
  - 하나의 프로세스 내의 다른 스레드 간의 동기화에 사용
  - `wait()` : 호출한 스레드를 `조건 동기 큐`에 삽입 (대기)
  - `notify()` : `조건 동기 큐`에 있는 하나의 스레드를 깨움
  - `notifyAll()` : `조건 동기 큐`에 있는 모든 스레드 깨움
- 식사하는 철학자
  1. 왼쪽 젓가락부터 집어든다. 다른 철학자가 이미 왼쪽 젓가락을 쓴다면 대기한다.
  2. 왼쪽을 들었다면 오른쪽 젓가락을 든다. 들 수 없다면 대기한다.
  3. 두 젓가락을 모두 들었다면 일정 시간 동안 식사를 한다.
  4. 식사를 마쳤으면 오른쪽 젓가락을 내려놓고, 왼쪽 젓가락을 내려놓는다.
  5. 다시 생각하다가 배고프면 1번으로 돌아간다.
- 식사하는 철학자 (교착상태) 해결 방법
  1. 최소 1명 이상은 다른 쪽 자원을 우선 요구 (팔 꼬기)
  2. 5명 중 최대 4명까지만 자원 요구
  3. 왼쪽과 오른쪽 자원을 모두 받거나, 아무것도 할당 받지 않고 대기하기
- Lamport의 빵집 알고리즘
  1. 직원 한 명이 5명의 손님을 대응
  2. 5명의 손님은 각각의 번호표를 발급받아서 번호표가 빠른 사람이 직원과 업무 처리
